...
class Pascal_122(Pascal2012):
    def __init__(self, root, split, transform_function):
        ...
        if split == "train_small":
            ...
            ind = rng.choice(len(self), 122, replace=False)
            self.img_names = np.array(self.img_names)[ind]
            self.cls_names = np.array(self.cls_names)[ind]
            self.mask_names = np.array(self.mask_names)[ind]
...
class Pascal_1226(Pascal2012):
    def __init__(self, root, split, transform_function):
        ...
        if split == "train_small":
            ...
            self.mask_names.sort()
            rng = np.random.RandomState(42)
            ind = rng.choice(len(self), 1226, replace=False)
            self.img_names = np.array(self.img_names)[ind]
            self.cls_names = np.array(self.cls_names)[ind]
            self.mask_names = np.array(self.mask_names)[ind]
...
# # class PascalOriginalSmall(PascalOriginal):
# #     def __init__(self, root, split, transform_function):
# #         if split == "train":
# #             split = "train_small"
# #         super().__init__(root, split, transform_function)
...
# #             np.random.seed(3)
# #             n_samples = int((883./10582)*len(self.img_names))
# #             ind = np.random.choice(len(self.img_names), n_samples, 
# #                                    replace=False)
# #             self.img_names = self.img_names[ind]
# #             self.cls_names = self.cls_names[ind]
# #             self.mask_names = self.mask_names[ind]
